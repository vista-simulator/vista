...
class Actor: # formerly World
    def __init__(self, trace_path, which_camera, camera):
        ...
        # Create synced and labeled timestamps for each trace
        ...
        (self.f_position, self.f_speed, self.f_curvature) = \
            self.get_interp_functions()
        self.curv_reset_probs = self.get_curv_reset_probs(
            self.current_trace_index, self.f_curvature)

        # Initialize stream
        video = os.path.join(trace_path, self.which_camera + '.avi')
        self.stream = FFReader(
            video,
            custom_size=(self.camera.get_height(), self.camera.get_width()),
            verbose=False)
    ...
    def reset(self):   #moved to Car
        ...
        self.curv_reset_probs = self.get_curv_reset_probs(
            ...
            self.f_curvature)  # Get new curv reset probs for it

        # Reset favoring places with higher curvatures
        self.current_frame_index = self.find_curvature_reset(
            self.curv_reset_probs)

        # First timestamp of trace
        self.first_time = self.masterClock.get_time_from_frame_num(
            self.which_camera,
            self.syncedLabeledFrames[self.current_trace_index][
                self.which_camera][self.current_frame_index])  # MODIFIED
    ...
    def get_interp_functions(self):  #moved to core.Trace
        ...
        odometry = np.genfromtxt(
            os.path.join(self.trace_path, TopicNames.odometry + '.csv'),
            delimiter=',')
        f_position = interp1d(
            odometry[:, 0], odometry[:, 1:3], axis=0, fill_value='extrapolate')
    ...
    def get_frame_from_time(self, worldtime):
        ...
        frame_to_worldtime = self.masterClock.cam_to_frame_time[
            self.which_camera]
        worldtime_to_frame = dict(
            [[v, k] for k, v in frame_to_worldtime.items()])
    ...
    def get_timestamp(self, index):
        if index >= len(
                self.syncedLabeledTimestamps[self.current_trace_index]) - 1:
            print("END OF TRACE")
            self.trace_done = True  # Done var will be set to True in deepknight env step
            return self.syncedLabeledTimestamps[self.current_trace_index][
                -1]  # Return last timestamp
    ...
    def spawn(self, trace_path):   #moved to core.Trace.getSyncedLabeledFrames
        ...
        for i in range(len(syncedLabeledTimestamps)):
            ...
            if syncedLabeledTimestamps[i] == syncedLabeledTimestamps[-1] or \
                ...
                syncedLabeledFrames = masterClock.get_frames_from_times(
