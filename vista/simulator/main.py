...
import util.drawing as draw
from util.ValueTracker import ValueTracker
...
class DeepknightEnv(gym.Env):
    ...
    def __init__(self, env_path, obs_size=None):
        ...
        which_camera = TopicNames.camera_front
        self.sess = tf.Session()
        self.camera = Camera(which_camera)
        if obs_size is None:
        ...
        for env in env_path:
            world = World(env, self.sess, which_camera, self.camera, VS)
            car = CarDynamics(world)
            self.worlds.append(world)
            self.cars.append(car)

        # Define action and observation spaces
        self.action_space = spaces.Box(low=LOWER_CURVATURE_BOUND, high=UPPER_CURVATURE_BOUND, shape=(1,), dtype=np.float32) #TODO: add speed as part of the action space?
        self.observation_space = spaces.Box(low=0, high=255, shape=(obs_size[0], obs_size[1], 3), dtype=np.uint8)
    ...
    def draw_gui(self, frame, info):
        ...
        steering_module = draw.draw_steering_wheel(WINDOW_SIZE, DRAWING_COORDS[draw.Module.STEERING], info['model_angle'])
        mini_car_on_road_module = draw.draw_mini_car_on_road(WINDOW_SIZE, DRAWING_COORDS[draw.Module.LANE], info['translation'], info['rotation'], self.world.road_width)
        info_module = draw.draw_info_sidebar(WINDOW_SIZE, DRAWING_COORDS[draw.Module.INFO], info)
        speed_module = draw.draw_speedometer(WINDOW_SIZE, DRAWING_COORDS[draw.Module.SPEED], model=2.23*info['human_velocity'], human=2.23*info['model_velocity'])
        frame_module = draw.draw_frame(WINDOW_SIZE, DRAWING_COORDS[draw.Module.FRAME], frame, self.world.camera, info['model_curvature'], info['human_curvature'])
