...
class CarDynamics:
    def __init__(self, world):
        ...
        '''
        self.world = world
        self.f_speed, self.f_distance, self.f_curvature = self.interpolate(self.world.trace_path)

        self.relative_state = State(translation_x=0.0, translation_y=0.0, theta=0.0)
    ...
    def interpolate(self, trace_path):
        ...
        '''
        speed = np.genfromtxt(os.path.join(trace_path, TopicNames.speed + '.csv'), delimiter=',')
        f_speed = interp1d(speed[:,0], speed[:,1], fill_value='extrapolate')

        distance = np.genfromtxt(os.path.join(trace_path, TopicNames.distance + '.csv'), delimiter=',')
        f_distance = interp1d(distance[:,0], distance[:,1], fill_value='extrapolate')

        # Human inverse_r from filtered odometry
        odometry = np.genfromtxt(os.path.join(trace_path, TopicNames.odometry + '.csv'), delimiter=',')
        f_omega = interp1d(odometry[:,0], odometry[:,4], fill_value='extrapolate')
    ...
    def step(self, curvature, velocity, delta_t):
        ...
        model_velocity = self.f_speed(current_timestamp) if velocity is None else velocity
        ...
        next_valid_timestamp, self.human_dynamics = self.world.get_next_valid_timestamp(
            human=self.human_dynamics,
            desired_ego=self.ego_dynamics)
        ...
        info = {
            'timestamp':        next_valid_timestamp,
            'first_time':       self.world.first_time,
            'human_curvature':  self.f_curvature(next_valid_timestamp),
            'human_velocity':   self.f_speed(next_valid_timestamp),
            'model_curvature':  model_curvature,
            'model_velocity':   model_velocity,
            'model_angle':      self.curvature_to_steering(model_curvature),
            'rotation':         self.relative_state.theta,
            'translation':      self.relative_state.translation_x,
            'distance':         self.f_distance(next_valid_timestamp)-self.f_distance(self.world.first_time),
    ...
    def compute_relative_transform(self):
        ...
        xy_global_centered = np.array([
            [ego_x_state - human_x_state],
