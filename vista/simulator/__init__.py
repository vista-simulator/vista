...
class Simulator(gym.Env):
    ...
    def __init__(self, env_path, obs_size=None):
        ...
        # Create list of worlds if only one passed in
        ...
        for env in env_path:
            world = World(env, TopicNames.camera_front, self.camera)
        ...
        self.action_space = gym.spaces.Box(
            low=LOWER_CURVATURE_BOUND,
            high=UPPER_CURVATURE_BOUND,
            shape=(1, ),
            dtype=np.float64)  #TODO: add speed as part of the action space?
    ...
    def reset(self):
        ...
        starting_timestamp, self.car.human_dynamics = self.world.get_next_valid_timestamp(
            human=self.car.human_dynamics, desired_ego=self.car.ego_dynamics)
    ...
    def step(self, action):
        ...
        translated_frame = self.world.synthesize_frame(frame, info,
                                                       self.car.relative_state)
    ...
    def draw_gui(self, frame, info):
        ...
        mini_car_on_road_module = draw.draw_mini_car_on_road(
            ...
            info['rotation'], self.world.road_width)
        ...
        frame_module = draw.draw_frame(
            ...
            self.world.camera, info['model_curvature'],
    ...
    def get_text_representation(self):
        ...
        loc = int(
            (float(self.car.position[1] / self.world.road_width + 0.5) * slots)
            + 1)  # Plus one for this character *
        if loc > slots:
            loc = slots
        if loc < 1:
            loc = 1
    ...
    def find_env_reset(self):
        ...
        new_env = np.random.choice(
            env_reset_probs.shape[0], 1, p=env_reset_probs)
        return new_env[0]
