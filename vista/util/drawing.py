...
with resources_root as rr:
    wheelImg = cv2.resize(cv2.imread(os.path.join(rr, 'img/mit.jpg')),
                          (360, 360))
    carImg = cv2.resize(cv2.imread(os.path.join(rr, 'img/lambo_square.jpg')),
                        (140, 300))
    roadImg = cv2.resize(cv2.imread(os.path.join(rr, 'img/road.jpg')),
                         (500, 300))
    speedImg = cv2.resize(cv2.imread(os.path.join(rr, 'img/speed.jpg')),
                          (800, 400))
'''
...
class Box:
    ...
    def scale(self, factor):
        ...
        )  #TODO: might be better to just modify the existing self instead of returning a new Box
    ...
    #this is only so previous code does not break...
    # TODO: will remove once old code changes to start using boxes like `draw_steering_wheel`.
...
def resize_keep_ratio(img, sz):
    ...
    if desired_ratio > box_ratio:
        ...
        return cv2.copyMakeBorder(bounded_resize,
                                  top=pad_top,
                                  bottom=pad_bottom,
                                  right=0,
                                  left=0,
...
def draw_mini_car_on_road(window_size,
                          box,
                          translation,
                          car_rotation,
                          road_width,
    ...
    M = cv2.getRotationMatrix2D((cols / 2, rows / 2),
                                car_rotation * 180 / math.pi, 1)
    car_rotated = cv2.warpAffine(car_resized, M, (cols, rows))

    #road image work
    if road:
        mini_car_on_road = resize_keep_ratio(roadImg,
                                             (window_width, window_height))
        road_shape = mini_car_on_road.shape  #because width of image doesn't necessarily equal width of window
        red_zone = (road_shape[1] / 10.
                    ) / road_shape[1]  #adjust here the actually red lane size
        scalar = 1 - 2. * red_zone - 2 * (
            float(mini_car_width) /
            road_shape[1]) - .2  #the extra .2 is because I'm not sure why
        ...
        mini_car_on_road[mini_car_height -
                         mini_car_height / 2:window_height / 2 -
                         mini_car_height / 2 + mini_car_height,
                         car_center - mini_car_width / 2:car_center -
                         mini_car_width / 2 + mini_car_width] = car_rotated
    else:
        ...
        relative_car_center = int(
            ((float(translation) / road_width) + .5) *
            mini_free_space)  #localized within mini_free_space
        ...
        mini_car_on_road[y_bot:y_top - adjust_y,
                         x_l:x_r - adjust_x] = car_rotated
        ...
        for i in range(0, window_height, stripe_gap):
            cv2.rectangle(mini_car_on_road, (mini_car_width // 8, i),
                          (mini_car_width // 4, i + stripe_length),
                          (255, 255, 255), -1)
        for i in range(0, window_height, stripe_gap):
            cv2.rectangle(
                mini_car_on_road, (window_width - mini_car_width // 8, i),
                (window_width - mini_car_width // 4, i + stripe_length),
                (255, 255, 255), -1)

    return mini_car_on_road


def draw_info_sidebar(window_size, box, time_now, time_start, model_angle,
                      human_curvature, model_curvature, distance,
                      model_velocity, translation, rotation):
    window_box = box.scale(window_size)
    width = int(window_box.get_width())
    height = int(window_box.get_height())
    ...
    elapsed_time = f"Elapsed Time: {time_now - time_start :.1f} sec"
    current_time = "Current Frame: "  #+ str(self.current_frame_index)
    steering_angle = f"Steering Angle: {model_angle :.1f} deg"
    actual_curv = f"Human Curvature: {human_curvature:.6f}"
    predicted_curv = f"Model Curvature: {model_curvature:.6f}"
    distance = f"Distance: {distance:.2f} m"
    velocity = f"Velocity: {2.23*model_velocity :.1f} mph"
    translation = f"Translation: {translation:.6f} m"
    rotation = f"Rotation: {rotation:.6f} rad"

    def putTextSidebar(text, i):
        return cv2.putText(info_sidebar, text,
                           (int(width / 20), int(i * height / 10)),
                           cv2.FONT_HERSHEY_SIMPLEX, width / 500.,
                           (255, 255, 255), 1)

    #apply text to array
    putTextSidebar(elapsed_time, 1)
    putTextSidebar(current_time, 2)
    putTextSidebar(steering_angle, 3)
    putTextSidebar(actual_curv, 4)
    putTextSidebar(predicted_curv, 5)
    putTextSidebar(distance, 6)
    putTextSidebar(velocity, 7)
    putTextSidebar(translation, 8)
    putTextSidebar(rotation, 9)
    # cv2.putText(info_sidebar, elapsed_time,
    #             (int(width / 20), int(height / 10)), cv2.FONT_HERSHEY_SIMPLEX,
    #             width / 500., (255, 255, 255), 1)
    # cv2.putText(info_sidebar, current_time,
    #             (int(width / 20), int(2 * height / 10)),
    #             cv2.FONT_HERSHEY_SIMPLEX, width / 500., (255, 255, 255), 1)
    # cv2.putText(info_sidebar, steering_angle,
    #             (int(width / 20), int(3 * height / 10)),
    #             cv2.FONT_HERSHEY_SIMPLEX, width / 500., (255, 255, 255), 1)
    # cv2.putText(info_sidebar, actual_curv,
    #             (int(width / 20), int(4 * height / 10)),
    #             cv2.FONT_HERSHEY_SIMPLEX, width / 500., (255, 255, 255), 1)
    # cv2.putText(info_sidebar, predicted_curv,
    #             (int(width / 20), int(5 * height / 10)),
    #             cv2.FONT_HERSHEY_SIMPLEX, width / 500., (255, 255, 255), 1)
    # cv2.putText(info_sidebar, distance,
    #             (int(width / 20), int(6 * height / 10)),
    #             cv2.FONT_HERSHEY_SIMPLEX, width / 500., (255, 255, 255), 1)
    # cv2.putText(info_sidebar, velocity,
    #             (int(width / 20), int(7 * height / 10)),
    #             cv2.FONT_HERSHEY_SIMPLEX, width / 500., (255, 255, 255), 1)
    # cv2.putText(info_sidebar, translation,
    #             (int(width / 20), int(8 * height / 10)),
    #             cv2.FONT_HERSHEY_SIMPLEX, width / 500., (255, 255, 255), 1)
    # cv2.putText(info_sidebar, rotation,
    #             (int(width / 20), int(9 * height / 10)),
    #             cv2.FONT_HERSHEY_SIMPLEX, width / 500., (255, 255, 255), 1)
...
def draw_speedometer(window_size, box, human=None, model=None):
    ...
    for i, speed in enumerate(speeds):
        ...
        end = (origin[0] + int(R * np.cos(angle)),
               origin[1] - int(R * np.sin(angle)))
        cv2.line(image, tuple(origin), tuple(end), color[i],
                 image.shape[0] // 80)
...
def draw_frame(window_size, box, frame, camera, model_projection,
               true_projection):
    window_box = box.scale(window_size)
    width = int(window_box.get_width())
    height = int(window_box.get_height())
    ...
    frame = Image.draw_box_new(frame,
                               camera.get_roi(),
                               linewidth=int(width / 90.))

    uncropped_height, uncropped_width = frame.shape[:2]
    new_frame = Image.draw_car_hood(frame)
    new_frame = new_frame[
        int(0.2 * uncropped_height):-int(0.2 * uncropped_height),
        int(0.2 * uncropped_width):
        -int(0.2 *
             uncropped_width)]  #this crops only the relevant part of the frame
...
def fuse(
    ...
    coordinates_list,
    draw_outline=True
):  #order of modules: steering wheel, road, info sidebar, frame #window_size passed in as X Y Z
    window = np.zeros(((window_size[1], window_size[0], window_size[2])),
                      np.uint8)
    for i in range(len(modules_list)):
        top_left = int(coordinates_list[i][0][0] * window_size[0]), int(
            coordinates_list[i][0][1] * window_size[1])
        bot_right = int(coordinates_list[i][1][0] * window_size[0]), int(
            coordinates_list[i][1][1] * window_size[1])
        adjust_x = 1 if modules_list[i].shape[
            1] != bot_right[0] - top_left[0] else 0
        adjust_y = 1 if modules_list[i].shape[
            0] != bot_right[1] - top_left[1] else 0
        window[top_left[1]:bot_right[1] - adjust_y,
               top_left[0]:bot_right[0] - adjust_x] = modules_list[i]
        if draw_outline:
            #draw rectange over the module for debugging
            box = Box(
                coordinates_list[i][0], coordinates_list[i][1]
            )  #TODO dont do this! just pass in the dictionary of boxes instead of coordinates_list (see steering module example)
            window_box = box.scale(window_size)
            window_box.round()
