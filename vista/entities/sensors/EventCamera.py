...
class EventCamera(BaseSensor):
    def __init__(self, attach_to: Entity, config: Dict) -> None:
        ...
        self._config['use_synthesizer'] = self._config.get('use_synthesizer', True)
    ...
    def reset(self) -> None:
        ...
        if self._config['use_synthesizer']:
            ...
            logging.debug('Not sure if it is ok to not close the event data stream')

            raw_path = os.path.join(self.parent.trace.trace_path, '{}.raw'.format(self.name))
    ...
    def capture(self, timestamp: float, 
        ...
        if self._config['use_synthesizer']:
            ...
            if self._prev_timestamp is not None:
                ...
                logging.debug('No dedicated alignment between ros and event camera clocks')
                offset_timestamp = multi_sensor.get_time_from_frame_num(ref_sensor, 0)

                event_timestamp_us = int((self._prev_timestamp - offset_timestamp) * 1e6)
                ...
                # TODO: hardcode original event camera image resolution
                logging.debug('Hardcoded event camera image resolution')
                ori_cam_h = 480
                ori_cam_w = 640
                cam_h = self.camera_param.get_height()
                cam_w = self.camera_param.get_width()
                ...
                subsample_factor = (cam_h * cam_w) / float(ori_cam_h * ori_cam_w) * 0.5 # TODO: 0.5 hacky
                n_events = events['x'].shape[0]
                subsample_mask = np.random.choice(np.arange(n_events), 
                                                  size=int(n_events * subsample_factor), 
                                                  replace=False)
                ...
                positive_events = np.array([events_y[positive_mask],
                                            events_x[positive_mask]]).T
                negative_events = np.array([events_y[negative_mask], 
                                            events_x[negative_mask]]).T
