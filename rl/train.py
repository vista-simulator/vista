...
from utils.callbacks import set_callbacks
...
def parse_args():
    ...
    parser = make_parser(
        ...
        description='Train a reinforcement learning agent using rllib.')
    parser.add_argument(
        '-f',
        '--config-file',
        default=None,
    ...
    parser.add_argument(
        '--temp-dir',
        default=os.environ.get('RAY_TMPDIR', '/tmp/ray'),
        type=str,
...
def main():
    ...
    path_keys = ['local_dir', 'config:env_config:trace_paths', 
                 'config:env_config:sensors_config1:rig_path'] # TODO: hacky
    for k in path_keys:
        try:
    ...
    if args.job_array_module:
        exp, exp_name = misc.update_by_job_array_exp(
            exp, args.job_array_module, args.job_array_task_id)
    else:
    ...
    # Resume training (restore from the latest checkpoint
    if args.resume:
